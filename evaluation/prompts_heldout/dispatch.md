...
=== WORKED EXAMPLE 2 — another SELECTION brief ===
BRIEF:
A high-end jeweler in Zurich stocks a retail vault from the wholesale inventory in diamonds.csv; each row is one stone available at wholesale cost. Deploy a fixed line of credit of CHF 250,000: the total wholesale price of the stones bought must not exceed it. The retail vault has 60 individual display slots, so at most 60 stones can be held. Total displayed stone volume cannot exceed 9,000 cubic millimetres; a stone's volume is x times y times z. No single cut grade may exceed 25% of the stones held. Carry at least two stones of every clarity grade so the range stays saleable. At most 40% of deployed capital may sit in stones priced above CHF 8,000. The goal is to maximise the total carat mass held in the vault. Model only a fixed sample of the inventory: 800 rows, random seed 7.
...
IR:
{
 ...
 "constraints": [
  ...
  {
   "name": "cut_concentration",
   "source_text": "No single cut grade may exceed 25% of the stones held.",
   "forall": [
    "CUT"
   ],
   "lhs": {
...
BRIEF:
A ride-hailing platform is dispatching a busy shift. The operational log taxis.csv holds completed trips; treat each row in the shift window as a pending customer request, with its pickup and dropoff timestamps, its passenger count, and the fare in the `total` column as the revenue it would earn. Four drivers are on shift: d1, d2, d3 and d4. A customer request can be given to at most one driver, and some requests will go unserved. No driver may receive overlapping schedules — a trip occupies its driver from its pickup time until its dropoff time. Passenger counts must never exceed the four-seat vehicle limit. Assign requests to drivers so as to maximise total platform revenue. Work only the shift running from 08:00 to 09:00 on 2019-03-06.
